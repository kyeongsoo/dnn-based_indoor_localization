...
#        building and floor and regression of location (reference point)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        help=
        "comma-separated numbers of units in hidden layers for building; default is '1024,1024'",
        default='1024,1024',
        type=str)
    parser.add_argument(
        "--floor_hidden_layers",
    ...
    parser.add_argument(
        ...
        help=
        "comma-separated numbers of units in additional hidden layers for location; default is '128'",
        default='128',
        type=str)
    parser.add_argument(
    ...
    print(
        "\nPart 2.4: buidling and training a hybrid building-floor-location classifier and location coordiates regressor ..."
    )
    x = loc_hl_output
    ...
    for units in coordinates_hidden_layers:
        ...
        x = Activation('relu')(x)
        x = Dropout(dropout)(x)
    x = Dense(utm.shape[1], kernel_initializer='normal')(x)
    x = BatchNormalization()(x)
    crd_output = Activation(
        'linear', name='coordinates_output')(x)  # 'linear' activation

    bflc_model = Model(
        inputs=input, outputs=[bld_output, flr_output, loc_output, crd_output])
    bflc_model.compile(
        optimizer=optimizer,
        loss=[
            'categorical_crossentropy', 'categorical_crossentropy',
    ...
    elapsedTime = timer() - startTime
    print(
        "Building-floor-location classifier and coordiates regressor trained in %e s."
        % elapsedTime)
    ...
    preds = bflc_model.predict(rss, batch_size=batch_size)
