...
import sys
sys.path.insert(0,
                os.path.expanduser('~kks/Research/Ongoing/localization/elsevier_nn_scalable_indoor_localization/program/models'))
...
session_conf = tf.ConfigProto(intra_op_parallelism_threads=1,
                              inter_op_parallelism_threads=1)  # force TF to use single thread for reproducibility
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        "--dropout",
        help=
        "dropout rate before and after hidden layers; default is 0.0",
        default=0.0,
        type=float)
    # parser.add_argument(
    #     "-N",
    ...
    model = KerasClassifier(build_fn=siso_dnn_classification, input_dim=num_aps,
                            ...
                            optimizer='adam', dropout=dropout, epochs=epochs,
    ...
    print("- Accuracy: {0:.2f}% ({1:.2f}%)".format((100*results.mean()), (100*results.std())))
    ...
    base_file_name = base_dir + "/E{0:d}_B{1:d}_D{2:.2f}_H{3:s}".format(epochs,
                                                                        batch_size,
                                                                        dropout,
                                                                        args.hidden_layers.replace(',',
                                                                                                   '-'))
